...
export default function Reports() {
  ...
  const reportItems = reportPhoto.map((item) => (
    <ReportItem
      ...
      reportPhoto={reportPhoto}
      src={item.photo}
      alt={`report-${item.id}`} // Виправлення: використано шаблонну стрічку для правильного форматування alt
      id={item.id.toString()}
    />
  ));
  ...
  return (
    <main className="relative">
      <RoutUnderHeader title="our reports" banner={reportBanner} />
      <section className="flex flex-col items-center justify-center self-stretch px-4 py-8 gap-[60px]">
        <div className="w-full flex flex-row justify-center flex-wrap">{reportItems}</div>
        <ReportModal reportPhoto={reportPhoto} />
      </section>
    </main>
  );
