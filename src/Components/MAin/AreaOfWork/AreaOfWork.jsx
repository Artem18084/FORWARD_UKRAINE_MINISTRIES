...
export default function AreaOfWork() {
  ...
  const mainTitle = currentTextData.main.areaOfWork.title;
  const titleWar = currentTextData.main.areaOfWork.contentData[0].title;
  const textWar = currentTextData.main.areaOfWork.contentData[0].text;

  const titleVolunteer = currentTextData.main.areaOfWork.contentData[1].title;
  const textVolunteer = currentTextData.main.areaOfWork.contentData[1].text;

  const titleYouth = currentTextData.main.areaOfWork.contentData[2].title;
  const textYouth = currentTextData.main.areaOfWork.contentData[2].text;
    ...
    <div className="flex items-center justify-center">
      <article className="flex flex-col  gap-8 md:gap-12 lg:gap-[60px]  py-12 md:py-[72px] lg:py-24 px-4 md:px-9 lg:px-11 xl:px-[72px] bg-white max-w-[1440px]">
        <section className="flex flex-col items-center gap-2">
          <Title title={mainTitle} color="black" />
          <div className="w-[103.5px] h-[1px] bg-[#4177ec]"></div>
        </section>
        <ul className="flex flex-col md:flex-row gap-6">
          {contentData.map((data, index) => (
            <AreaOfWorkCard key={index} alt={data.alt} contentData={data} />
          ))}
        </ul>
      </article>
    </div>
